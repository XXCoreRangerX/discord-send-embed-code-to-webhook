          ...
          if(selection.length > this.discordCharLimit) { // If content has more chars than discord char limit
            ...
            notification = atom.notifications.addWarning(
              ...
              buttons: [
                {
                  onDidClick: function() {
                    sendFunction(selection.substring(0, charLimit), grammar, fileName, webhookID, webhookToken, webhookUsername, webhookAvatar, webhookTitle, webhookAuthor, webhookAuthorAvatar, webhookAuthorURL, webhookColor, webhookFooterText, webhookFooterImage),
            ...
            this.sendtowebhook(selection, grammar, fileName, webhookID, webhookToken, webhookUsername, webhookAvatar, webhookTitle, webhookAuthor, webhookAuthorAvatar, webhookAuthorURL, webhookColor, webhookFooterText, webhookFooterImage);
          }
        } else {
          atom.beep()
...
sendtowebhook(selection, grammar, fileName, webhookID, webhookToken, webhookUsername, webhookAvatar, webhookTitle, webhookAuthor, webhookAuthorAvatar, webhookAuthorURL, webhookColor, webhookFooterText, webhookFooterImage) {
  const webhookClient = new Discord.WebhookClient(webhookID, webhookToken);
  async function asyncCall() {
    const fileExtension = fileName.split('.').pop();
    const hastebinLink = await hastebin(selection, { url: "https://paste.mod.gg", extension: fileExtension });
