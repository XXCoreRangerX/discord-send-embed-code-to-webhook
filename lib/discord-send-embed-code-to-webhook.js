...
export default {
  config: {
    ...
    "webhook-title-url": {
      "description": "The on-click title URL of the webhook.",
      "type": "string",
      "default": ""
    },
    "webhook-author": {
    ...
    "webhook-author-avatar": {
      "description": "An avatar of the webhook's author (useful for larger teams).",
      "type": "string",
      "default": ""
    },
    "webhook-author-url": {
      "description": "A URL of the webhook's author.",
      "type": "string",
      "default": ""
    },
    "webhook-color": {
  ...
  sendSelectionToDiscord(webhookConfigToUse) {
    ...
    if(editor = atom.workspace.getActiveTextEditor()) {
      ...
      if(selection && selection != "") {
        ...
        let webhookTitleURL = atom.config.get('discord-send-embed-code-to-webhook.webhook-title-url');
          ...
          if(selection.length > this.discordCharLimit) { // If content has more chars than discord char limit
            ...
            notification = atom.notifications.addWarning(
              ...
              buttons: [
                {
                  onDidClick: function() {
                    sendFunction(selection.substring(0, charLimit), grammar, webhookID, webhookToken, webhookUsername, webhookAvatar, webhookTitle, webhookTitleURL, webhookAuthor, webhookAuthorAvatar, webhookAuthorURL, webhookColor, webhookFooterText, webhookFooterImage),
            ...
            this.sendtowebhook(selection, grammar, webhookID, webhookToken, webhookUsername, webhookAvatar, webhookTitle, webhookTitleURL, webhookAuthor, webhookAuthorAvatar, webhookAuthorURL, webhookColor, webhookFooterText, webhookFooterImage);
          }
        } else {
          atom.beep()
...
sendtowebhook(selection, grammar, webhookID, webhookToken, webhookUsername, webhookAvatar, webhookTitle, webhookTitleURL, webhookAuthor, webhookAuthorAvatar, webhookAuthorURL, webhookColor, webhookFooterText, webhookFooterImage) {
  const webhookClient = new Discord.WebhookClient(webhookID, webhookToken);
	const embed = new Discord.MessageEmbed()
		.setTitle(webhookTitle)
    .setURL(webhookTitleURL)
